...
**1.** How have the variable types changed when using `read_csv` instead of `read.csv`?
    ...
    **Timestamp is chr in energy 1. Timestamp is dttm in energy 2.**
...
Because the first TK lines are 'NA'... Read_csv takes the first 1000 entries into account.
...
It's more wide than narrow, so the data isn't tidy. The rows are data and the columns aretime and building name.

**3.** How many buildings on campus are in the energy data?


90 (total columns) - 8 (timestamp, year, month, weekOfYear, dayOfMonth, dayWeek, timeHour, timeMinute) = 82.
