...
Carrier/airlines connect the two datasets 

- What variable(s) are the keys that connect the `flights` and `airports` data sets?
Origin & destination ... FAA

#### 2. Merging  `airports` and `flights` data. 

- How many unique flight destintations in `flights` are not given in the `airports` data? (Hint: look at `anti_join` or `semi_join`)

```{r}

anti_join(x,y, by=c("x_key"="y_key"))

```



- How many airports given in `airports` are not destinations given in `flights`?
- Use `left_join` to add `airports` info to the `flights` data. How many rows are there in this joined data? Then repeat using `inner_join` and `full_join`. Explain the differences in row counts for these three ways of joining. 
